...
class Settings(BaseSettings):
    ...
    #  1) 색인 동기화의 **보장 수준**. 오답 방향은 이미 구조적으로 막혀 있다 — 검색 결과를 PG로
    #     되물어 걸러내므로(1층) 삭제·비공개 문서는 색인이 낡아도 인용되지 않는다(실측 확인).
    #     남은 것은 누락 방향이다: 이중 쓰기(2층)가 최선노력이라 OpenSearch 장애 중의 업로드는
    #     `python -m eval._os_index --repair`(3층)까지 검색에 안 잡힌다. "업로드 후 N초 내 검색"이
    #     요구사항이 되면 outbox 테이블 + arq 드레인으로 승격해야 한다.
