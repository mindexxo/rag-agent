...
judge = Gemini 2.5 Flash (외부 강모델, self-judge 편향 회피)
embeddings = 로컬 BGE-M3 재활용 (answer_relevancy용, 외부 콜 절약)

metric (reference 불필요한 2축부터):
- faithfulness      : 답변 주장이 retrieved_contexts에 근거하는가 (환각)
- answer_relevancy  : 답변이 질문에 맞는가

실행: python -m eval.ragas_eval            # SMOKE=3 (기본, rate limit 안전)
      SMOKE=0 python -m eval.ragas_eval    # 전체 120
"""
import os
...
def main():
    print(f"judge={JUDGE_MODEL}  |  SMOKE={SMOKE or '0(전체)'}")
    r = compute()
    print(r["result"])
    print(f"→ saved {r['csv']}")
