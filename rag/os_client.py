"""OpenSearch 연결·인덱스 매핑 — 0층 leaf(톱레벨은 config만). #139 도입, #146에서 분리.

청크(본문·메타·벡터·어휘 필드)는 OpenSearch에만 있다. PG는 문서·FAQ·폴더·대화·캐시·outbox의 정본이고
청크 행을 쓰지 않는다. 읽기 경로는 `rag/os_search.py`, 쓰기 경로(정합 관리)는 `rag/os_index.py`,
재동기화는 `rag/os_reconcile.py`. 이 모듈은 클라이언트 싱글톤·인덱스 보장·**매핑의 정의점**이다.
...
k1 차이는 리랭커 뒤에서 흡수된다. bigram 토큰은 analyzer로 재현하지 않는다(`LEX_BIGRAM_FIELD` 주석).

매핑은 인덱스 생성 시점에 굳는다 — 바꾸면 새 인덱스 + 전량 재색인이다(ensure_index는 있는 인덱스를 건드리지 않는다).
"""
import logging
...
def client():
    ...
    `opensearchpy`는 함수 안에서 import한다 — 선택 의존이라, 매핑 상수만 쓰는 순수 테스트가
    클라이언트 없이도 이 모듈을 import할 수 있게(유일하게 남는 지연 import).
