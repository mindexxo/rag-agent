"""OpenSearch 읽기 경로 — 후보 회수 두 개(dense kNN·BM25 Nori)와 본문 조회(mget). #146에서 분리.

`rag/retriever.py`가 이 셋만 부르고, 그 위(멀티쿼리 RRF·리랭커·표 필터·top_n·게이트)는 엔진과 무관하다.
...
- **청크 본문·메타는 엔진에서 읽는다**(`fetch_chunk_map`) — 실무 표준 구성. 왕복 한 번으로
  ...
  낡은 텍스트·메타가 인용된다. 그 대가를 쓰기 경로가 갚는다(`rag/os_index.py` 상단 "정합 관리").
...
async def fetch_chunk_map(ids: list[int]) -> dict:
    ...
    쓰기 경로가 정합을 책임진다(rag/os_index.py 상단 "정합 관리"): 삭제·메타 변경은 즉시 부분
