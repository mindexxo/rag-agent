...
규약(#62): **_SYSTEM_PROMPT_TEMPLATE 본문에 번호 참조를 새로 늘리지 않는다.**
규칙이 늘거나 줄면 재번호가 따르고 번호 참조는 그때 조용히 낡는다 — 모델이 없는 규칙을
찾게 되니 주석이 낡는 것과 등급이 다른 문제다. #62 재번호 때 규칙 3의 "규칙 7의 마지막 줄"이
실제로 그렇게 낡아(재번호 후 규칙 7은 민감정보 규칙) "아래 인용 표시 규칙"으로 고쳤다.
새 의존은 그 자리에서 다시 서술한다.

남아 있는 참조는 규칙 3의 "규칙 2에 따른" 한 곳이다. 지우지 않은 이유: 규칙 2는 위쪽이라
번호가 밀릴 여지가 작고, **문구 수정은 그 자체로 재측정 대상**이라 근거 없이 건드리지
않는다(#62에서 문구 한 줄 차이가 trap 1문항을 3/3 뒤집었다).
test_prompts.py의 래칫 테스트가 이 1곳만 허용하고 새 참조를 막는다.
...
# ⚠ 규칙 3은 축약하지 말 것 (#62 실측·기각). 규칙 8의 첫 불릿("첫 문장은 핵심 답 한 줄로")을
#   지운 것과 짝이므로 그 불릿도 되살리지 말 것 — 되살리면 첫 문장이 자기모순을 낸다
#   (본문은 맞는데 앞에 헛거절을 붙인다, #48·#62 실측).
#
#   규칙 3 축약을 후보 6개로 시도해 전부 기각했다(8줄 → 3~7줄). 원리형 재작성 5개는
#   미거부가 1~2건에서 3~9건으로 올랐고 환각 실물이 나왔다 — 없는 AS센터 전화번호 생성,
#   없는 오프라인 반품 규정 생성. 금지어 열거 축소·부정구 삽입·"목록 부재 ≠ 제외" 원리
#   어느 것도 레버가 아니었다(금지어 3개인 후보가 0개인 후보보다 나빴다).
#   마지막 한 줄만 지운 후보(H11)는 전 축 노이즈였으나 trap 1문항이 3/3 재현 회귀했다 —
#   "특정 표현이 문서에 없다는 이유만으로 거절하지 마십시오"가 그 방어선이다.
#   조건별 원문·지표는 eval/results/prompt_ablation_62/ (9_최종요약.md부터 볼 것).
#
#   다시 손대려면 전제가 둘이다. (1) 이 8줄은 서로 물려 있어 한 줄씩 빼는 방식으로는
#   원인이 안 잡힌다. (2) 지금 지표에 판정 아티팩트가 섞여 있다 — #61 규약이 "실인용 0건 =
#   거절"이라 본문을 안 보므로, 맞게 답하고 꼬리만 빈 목록인 출력이 거절로 집계된다.
#   그 아티팩트를 먼저 걷어내야 축약 실험이 신호를 낸다.
#
