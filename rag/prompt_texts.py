...
# 출처 꼬리 마커 (#56) — 답변 끝에 {TAIL_START}[라벨]…{TAIL_END} 형태로 붙는다.
# '§§'는 한국어 상담 답변·문서에 등장할 확률이 사실상 0이라 본문 오탐을 피한다.
...
7. 답변 본문에는 인용 표기를 넣지 마십시오. 답변을 모두 마친 뒤, 마지막 줄에 실제로 근거로 사용한 문서만
   {TAIL_START}[라벨]{TAIL_END} 형식으로 정확히 한 번 표시하십시오.
   라벨은 <문서>/<첨부 문서> 블록 앞에 붙은 형식 그대로입니다 — [파일명 v{{version}}], [FAQ], [첨부: 파일명].
   서로 다른 문서의 규정을 함께 사용해 답했다면 라벨을 이어서 모두 적으십시오. 예: {TAIL_START}[a.pdf v1][FAQ]{TAIL_END}
...
9. 답변 서식 — 상담원이 한눈에 읽을 수 있게 작성하십시오:
   ...
   - 단순변심: 고객 부담 (왕복 배송비)
   - 상품 하자·오배송: 회사 부담
   {TAIL_START}[반품정책.pdf v1]{TAIL_END}
"""
