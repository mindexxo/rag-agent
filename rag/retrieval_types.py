"""검색 결과 자료형 — 0층 leaf. `rag/retriever.py`(조립)와 `rag/os_search.py`(엔진 읽기)가 함께 쓴다.

여기 둔 이유(#146): 예전엔 `retriever.py`에 있어서 엔진 모듈이 이 자료형을 만들려면 retriever를
함수 안에서 지연 import해야 했다(retriever→엔진, 엔진→retriever의 순환). 자료형을 leaf로 빼면
양쪽이 톱레벨에서 import한다. **소비자는 그대로 `from rag.retriever import RetrievedChunk`로 써도 된다** —
retriever가 톱레벨에서 import하므로 같은 객체다(정의점만 여기).
"""
