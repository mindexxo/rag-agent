...
class RetrievedChunk:
    ...
    branches: list[str]             # 'dense'(원본)/'sparse'(하이브리드)/'expand'(변형 쿼리, #3) 조합 — 어느 검색에서 잡혔는지, 디버깅·로깅용
...
def _rrf_fuse(
    ...
    """N개 순위 리스트를 RRF 점수로 합산. 쿼리 확장(#3) 융합에 사용 중이며,
...
async def retrieve_candidates(
    ...
    expanded_queries: list[str] | None = None,   # 쿼리 확장(#3) 변형 — 검색·RRF 융합 전용
) -> RetrievalCandidates:
    """하이브리드 검색까지만. 근거 게이트는 apply_gate가 담당.

    expanded_queries가 있으면 원본+변형을 각각 dense 검색해 RRF로 융합한다 (#3).
    ...
    # 변형 쿼리에서 잡힌 청크 — 어느 검색이 기여했는지 디버깅·로깅용 (#3)
...
async def retrieve(
    ...
    expanded_queries: list[str] | None = None,   # 쿼리 확장(#3) 변형 — retrieve_candidates로 전달만
