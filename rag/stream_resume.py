...
리미터가 Redis를 방어 없이 호출하므로(rag/limiter.py) Redis 장애 시 **새 턴**은 어차피 못 받는다.
하지만 **이미 생성 중인 턴**은 리미터를 이미 통과했다 — 단일 경로면 그 턴들의 스트림이
답변 중간에 끊긴다(사용자 눈앞에서 문장이 잘린다). 이중 쓰기는 인메모리가 계속 흘러 그게 안 보인다.
...
기존 Redis 사용 두 곳과 다른 세 번째 방침이라 근거를 남긴다.
  limiter        전파   — 삼키면 "Redis 다운 시 동시성 무제한 통과"가 된다
