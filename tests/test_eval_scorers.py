...
class TestCitationAccuracy:
    def test_전_기대문서_인용시_만점(self):
        score = citation_accuracy('답변 [환불정책.pdf v1] 그리고 [배송정책 v2]',
                                  ['환불정책.pdf', '배송정책.docx'])
        assert score == 1.0

    def test_multi_doc_부분_인용은_비율(self):
        # v2 개정의 핵심 — any-match였으면 1.0 과대평가
        score = citation_accuracy('답변 [환불정책.pdf v1]', ['환불정책.pdf', '배송정책.docx'])
        assert score == 0.5

    def test_FAQ_인용_미탐_해소(self):
        assert citation_accuracy('답변입니다 [FAQ]', ['FAQ']) == 1.0

    def test_오인용만_있으면_0(self):
        assert citation_accuracy('답변 [엉뚱문서.pdf v1]', ['환불정책.pdf']) == 0.0

    def test_인용_없으면_0(self):
        assert citation_accuracy('인용 없는 답변', ['환불정책.pdf']) == 0.0

    def test_corpus_v2_확장자_스트립(self):
        assert citation_accuracy('[공지 v1]', ['공지.txt']) == 1.0
        assert citation_accuracy('[멤버십혜택표 v1]', ['멤버십혜택표.xlsx']) == 1.0
    ...
    def test_대체_출처_그룹은_하나만_인용해도_인정(self):
        ...
        assert citation_accuracy('[멤버십혜택표 v1] [제휴카드결제안내 v1]', exp) == 1.0
        assert citation_accuracy('[멤버십포인트 v1] [제휴카드결제안내 v1]', exp) == 1.0
        assert citation_accuracy('[제휴카드결제안내 v1]', exp) == 0.5   # 그룹 미커버는 여전히 감점
