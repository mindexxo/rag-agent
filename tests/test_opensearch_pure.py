...
from rag import os_client, os_index, os_search
...
def test_bm25_hnsw_파라미터는_엔진_기본값을_쓴다():
    ...
    settings_ = os_client.MAPPING["settings"]["index"]
    assert "similarity" not in settings_
    assert not any(k.startswith("knn.algo_param") for k in settings_)
    props = os_client.MAPPING["mappings"]["properties"]
    for field in (os_client.NORI_FIELD, os_client.LEX_BIGRAM_FIELD):
...
def test_dense_필드는_임베딩_차원과_lucene_cosinesimil로_고정된다():
    ...
    dense = os_client.MAPPING["mappings"]["properties"]["dense"]
    assert dense["type"] == "knn_vector" and dense["dimension"] == os_client.DIM == 1024
...
def test_어휘_입력_조립이_운영과_같다():
    ...
    assert os_index.lex_text(text, fn, hp) == build_index_text(text, fn, hp)
    assert os_index.lex_text(text, None, None) == text          # FAQ — 프리픽스 없음
    # 폴더 설명은 들어가지 않는다 (리랭커 전용, rag/index_text.py docstring)
    assert "폴더" not in os_index.lex_text(text, fn, hp)
...
def test_bigram_필드는_공백만_자른다():
    ...
    props = os_client.MAPPING["mappings"]["properties"]
    analyzer = props[os_client.LEX_BIGRAM_FIELD]["analyzer"]
    assert os_client.MAPPING["settings"]["analysis"]["analyzer"][analyzer]["tokenizer"] == "whitespace"
...
def test_nori_질의에_2_18_결함_회피가_붙어_있다():
    ...
    clause = os_search.lex_clause(os_client.NORI_FIELD, "적립금 얼마")
    assert clause["match"][os_client.NORI_FIELD]["auto_generate_synonyms_phrase_query"] is False
...
def test_게이트_신호_환산이_실측과_같다():
    ...
    for score, pg_distance in ((0.862583, 0.274834), (0.849968, 0.300063),
                               ...
                               (0.830557, 0.338887)):
        assert abs(os_search.score_to_cosine_distance(score) - pg_distance) < 2e-6, score
...
def test_필터는_엔진에서_걸린다():
    ...
    두는 것이 그 대가다.
    """
    terms = os_search.tenant_filter('t1')['bool']['filter']
...
def test_searchable_판정이_조건표대로다():
    ...
    assert os_index.effective_searchable(**ok, folder_is_searchable=None) is True   # 미분류 문서
    assert os_index.effective_searchable(**ok, folder_is_searchable=True) is True
    assert os_index.effective_searchable(**ok, folder_is_searchable=False) is False  # 폴더 off
    assert os_index.effective_searchable(**{**ok, 'doc_is_active': False}) is False
    assert os_index.effective_searchable(**{**ok, 'doc_status': 'deleted'}) is False
    assert os_index.effective_searchable(**{**ok, 'doc_is_searchable': False}) is False
    assert os_index.effective_searchable(is_faq=True, faq_is_active=True) is True
    assert os_index.effective_searchable(is_faq=True, faq_is_active=False) is False
...
def test_벡터는_청크에_실려_온_값으로_색인된다():
    doc = os_index.build_doc(_Chunk(dense=[0.5] * 1024), '정책.pdf', 2)
...
def test_벡터가_없으면_크게_실패한다():
    ...
    with pytest.raises(ValueError, match='벡터가 없다'):
        os_index.build_doc(_Chunk(has_dense=False), '정책.pdf', 2)
    with pytest.raises(ValueError, match='벡터가 없다'):
        os_index.build_doc(_Chunk(dense=None), '정책.pdf', 2)


def test_어휘_필드는_본문에서_만든다():
    """Nori·bigram 필드는 lex_text(본문+프리픽스)에서 색인 시점에 조립된다."""
    ch = _Chunk(dense=[0.5] * 1024)
    doc = os_index.build_doc(ch, '정책.pdf', 2)
    expected = os_index.lex_text(ch.text, '정책.pdf', ch.heading_path)
    assert doc[os_client.NORI_FIELD] == expected
    assert doc[os_client.LEX_BIGRAM_FIELD] == ' '.join(lexical.bigrams(expected))


def test_chunk_os_id는_결정적이고_충돌이_없다():
    """PG 시퀀스 없이 (부모 id, chunk_index)로 만드는 id — 재색인이 같은 _id로 덮여 멱등하려면
    결정적이어야 하고, 문서·청크 조합마다 달라야 한다."""
    f = os_index.chunk_os_id
...
def test_chunk_os_id는_20비트_상한을_강제한다():
    ...
    assert os_index.chunk_os_id(document_id=5, chunk_index=(1 << 20) - 1) > 0
    with pytest.raises(ValueError, match='20비트'):
        os_index.chunk_os_id(document_id=5, chunk_index=1 << 20)
...
async def test_엔진에_못_붙으면_ensure_index가_예외를_올린다(monkeypatch):
    ...
    monkeypatch.setattr(os_client, 'client', lambda: _Client())
...
async def test_이미_있는_인덱스는_건드리지_않고_경합의_already_exists는_성공으로_본다(monkeypatch):
    ...
    monkeypatch.setattr(os_client, 'client', lambda: _Client(True))
    await os_client.ensure_index()
    assert calls == []                                   # 있으면 create를 부르지 않는다
    monkeypatch.setattr(os_client, 'client', lambda: _Client(False))
    await os_client.ensure_index()                               # 웹·워커 동시 생성 경합 — 예외 아님
    assert calls == ['create']
...
async def test_ensure_index_soft는_못_붙어도_예외_없이_False(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger='rag.os_client'):
        assert await os_client.ensure_index_soft() is False
