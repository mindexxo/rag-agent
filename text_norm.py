...
**에러 없이 조용히** 0이 됐다. (#56 개정으로 인용이 번호 기반이 되며 그 매칭 자체는
사라졌지만, 저장·조회·supersede 비교는 여전히 이 정책에 기댄다.)
...
예외: LLM 출력처럼 **우리가 통제할 수 없는 입력**과 텍스트를 비교하는 자리는 비교 시점에도
방어한다(`nfc()` — 현재는 eval의 폴더 배정만 해당). 저장값이 NFC여도 모델이 어느 형태로
낼지는 보장이 없다.
"""
import unicodedata
