...
LLM은 프롬프트의 그 라벨을 읽어 NFC로 출력했다. `cited_filenames()`의
`f'[{filename} v{version}]' in answer`가 못 찾아 `cited_docs`가 빈 배열이 되고,
FE 각주와 인용 지표가 **에러 없이 조용히** 0이 됐다.
...
(`rag/prompts.py`의 `cited_filenames`). 저장값이 NFC여도 모델이 어느 형태로 낼지는 보장이 없다.
"""
import unicodedata
